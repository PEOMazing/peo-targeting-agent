...
export function classifyIncumbent(org) {
  ...
  if (has('totalsource') || has('trinet') || has('insperity') || has('justworks') || has('oasis') || has('vensure') || has('sequoia') || has('amplify peo') || has('engage peo')) {
    let vendor = pick(tech, ['trinet', 'insperity', 'justworks', 'totalsource', 'oasis', 'vensure', 'sequoia']) || 'a PEO';
    if (vendor === 'Oasis' || has('paychex oasis')) vendor = 'Paychex Oasis';
    if (vendor === 'Totalsource') vendor = 'ADP TotalSource';
    return { category: 'PEO competitor', vendor, motion: 'Displacement' };
  }
  ...
  return { category: 'None detected', vendor: null, motion: 'Likely Multi-vendor' };
}
...
export function scoreCompany(org, extra = {}) {
  ...
  if (incumbent.category === 'Payroll (Gusto)') { s += 22; reasons.push('On Gusto payroll — prime payroll → PEO upsell.'); }
    ...
    if (e != null && e < 100) { s += 19; reasons.push('No HR/PEO vendor detected + sub-100 headcount — no single HR/PEO platform detected — likely juggling multiple vendors a PEO consolidates.'); }
    else { s += 12; reasons.push('No HR/PEO vendor detected.'); }
  }
  else if (incumbent.category === 'Modern HRIS' || incumbent.category === 'HRIS') { s += 12; reasons.push(`On ${incumbent.vendor || 'an HRIS'} — competitive upsell, not a fresh build.`); }
