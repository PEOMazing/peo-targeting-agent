...
export function classifyIncumbent(org) {
  ...
  return { category: 'None detected', vendor: null, motion: 'No incumbent' };
}
...
export function scoreCompany(org, extra = {}) {
  ...
  if (incumbent.category === 'Payroll (Gusto)') { s += 22; reasons.push('On Gusto payroll — prime payroll → PEO upsell.'); }
    ...
    if (e != null && e < 100) { s += 19; reasons.push('No HR/PEO vendor detected + sub-100 headcount — no current HR/PEO vendor — open opportunity.'); }
