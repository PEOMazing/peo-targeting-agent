...
export default function Page() {
  ...
  const score = async () => {
    ...
    setScoring(true); setError(''); setResults(null); setSelected(null); setBrief(null);
    try {
  ...
  const researchCompany = async () => {
    ...
    setResearching(true); setError(''); setResults(null); setSelected(null); setBrief(null); setEmailData(null);
    try {
      ...
      setResults([data.result]); setDemo(false);
      openRow(data.result, 0);
    } catch (e) { setError(e.message); } finally { setResearching(false); }
  };

  const openRow = async (r, i) => {
    setSelected(i); setBrief(null); resetFeedback(); setEmailData(null); setBriefLoading(true); setError('');
    try {
    ...
    <div className="wrap">
      ...
      {tab === 'target' && (
        ...
        <div className="grid">
          <div className="card">
            ...
            {results && (
              <div className="ranklist">
                <div className="rankhead">{results.length} ranked by PEO fit</div>
                {results.map((r, i) => (
                  <div key={i} className={selected === i ? 'rankrow on' : 'rankrow'} onClick={() => openRow(r, i)}>
                    <span className="rankscore" style={{ color: scoreColor(r.score) }}>{r.score ?? '—'}</span>
                    <span className="rankmain">
                      <span className="rankco">{r.name}</span>
          ...
          <div className="card">
            <h2>Upsell brief</h2>
            {selected == null && !briefLoading && <p className="empty">Score a list, then tap any company to enrich it into a full payroll → PEO upsell brief — the motion, fit rationale, the angle, talking points, objections, and the comp/benefits hook.</p>}
            ...
            {brief && sel && (
              <div className="brief">
                ...
                <div className="whyfit">
                  <div className="whyfithead">Why this score</div>
                  <ul className="points">{(sel.reasons || []).map((x, i) => <li key={i}>{x}</li>)}</ul>
                </div>
                ...
                <div className="blk"><h3>Fit</h3>
                  ...
                  <p style={{ marginTop: 12 }}>{brief.fitRationale}</p>
                </div>
                  ...
                  {emailData && (
                    <>
                      ...
                      <div className="timeline">
                        {(emailData.sequence || []).map((s, i) => (
                          <div className="tstep" key={i}>
                            <div className="tday">Day {s.day}</div>
                            <div className="tcard">
                              <div className="tpurpose">{s.purpose}</div>
                              <div className="emsub">{s.subject}</div>
                              <p className="embody">{s.body}</p>
                              <div className="emactions">
                                <span className="copybtn" onClick={() => copyText('s' + i, s.subject, s.body)}>{copied === 's' + i ? '✓ Copied' : 'Copy'}</span>
                                <a className="copybtn" href={mailto(s.subject, s.body)}>Open</a>
                              </div>
                            </div>
                          </div>
                        ))}
