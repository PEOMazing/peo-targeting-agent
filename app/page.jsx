  ...
  },
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        blocks: [
          { t: "p", x: "Module 3 taught the four places any PEO makes money: the admin fee, workers' comp pricing, unemployment taxes, and benefits rates. We earn in all of them, that's the business. The difference is we earn cleanly: one published, all-inclusive admin rate, with the client's true costs passing through untouched. Five rules define it." },
          { t: "list", items: ["One flat, all-inclusive PEPM admin fee. A single per-employee-per-month number, published and lookup-able, that covers every service we provide, never a percent of payroll. Raises and bonuses never change what we charge, because our fee has nothing to do with their wage bill.","Tier-priced, not deal-priced. The fee is set by tier (PEO 1, 2, 3) and headcount band, not negotiated case by case. Two similar companies get the same number. No sales-skill tax, no quote roulette.","Margin in the fee, not in the pass-throughs. The administrative spreads we manage (SUTA, ben-admin, comp admin, float) are real profit, but they live inside the tiered admin fee, not as hidden markups on the client's true costs. One transparent number for the work we do.","Comp and tax at true rate, spread disclosed. Where we administer workers' comp or unemployment, the client sees the real rate. We never bury margin in a blended comp or SUTA number.","Renewal philosophy in writing before signing. The client sees how renewals work and what drives them (medical trend and their own claims, not a sales lever) before the first invoice. We can't cap medical trend, but we can promise no surprises. Year two is where PEOs are won or lost." ] },{ t: "p", x: "Published pricing does in this category what it did for Gusto in payroll: it turns an opaque, quote-gated sale into one a buyer can start by themselves. The published number is a trust signal before a rep ever speaks." },
          { t: "callout", tag: "ALWAYS EXTRA: EVERYWHERE, INCLUDING HERE", x: "No PEO's admin fee, ours included, covers gross wages, employer taxes (FICA, FUTA, SUTA), health/dental/vision premiums, workers' comp premium, state-mandated coverages, or 401(k) employer contributions. Those are pass-throughs, billed at cost on their own lines. Naming them unprompted is part of how we sell." },
    ...
    quiz: [
      ...
      { q: "The non-negotiable rule of the internal co-sell motion:", opts: ["PEO reps work accounts secretly to move fast", "Joint selling on ICP-designated accounts, with clear rules of engagement and shared credit so the handoff is seamless", "The payroll team is cut out once PEO is identified", "Payroll AEs must not be told about PEO"], a: 1, x: "Accounts to be worked are designated by ICP, and the payroll and PEO teams sell them together under written rules of engagement: who owns what, how credit is shared, and how the handoff happens. That makes the payroll org our distribution engine instead of a competitor. Without those rules, the motion creates enemies inside our own building, the one channel conflict that can kill this product." },
    ...
    <div className="shell">
      ...
      <div className="phase">
        ...
        <ul>
          ...
          <li><b>One pricing model wraps all of it.</b> One flat published PEPM admin fee, set by tier and headcount band rather than negotiated deal by deal. Every true pass-through, wages, taxes, comp premium, benefit premiums, itemized at cost on its own line, never marked up. The margin lives in the tiered admin fee and the administrative spreads we manage (SUTA, ben-admin, comp admin, float), bundled into one transparent number rather than buried as hidden markups. Renewal philosophy, how increases are driven and allocated, in writing before signing.</li>
    ...
    <div className="shell">
      ...
      <div className="phase">
        ...
        <ul>
          ...
          <li><b>Internal partnership before external motion.</b> Week one belongs to peer sales leaders: written rules of engagement for install-base conversion, who owns the account, how credit is shared, when a payroll AE brings PEO in, so converting the base makes allies of the payroll org, never victims. Co-selling into a shared customer base only works when the peer team wins every time we win.</li>
  ...
  return (
    <div className="shell">
      ...
      <div className="phase">
        ...
        <ul>
          <li><b>Align with the sales org.</b> Co-design the co-sell rules of engagement with the payroll and benefits sales leaders, account ownership, shared credit, and the warm-handoff motion, so converting the base makes the peer team win every time we win.</li>
      ...
      <div className="phase">
        <div className="phase-head"><span className="phase-days">THE PEOPLE SIDE</span><h3>Building the team</h3></div>
        <p className="theme">The strategy only matters if the people running it are right. Here's how I hire, how I coach, and where I'd lean on HR.</p>
        <ul>
          <li><b>I hire on CHAMP, not the resume.</b> Coachable, Hardworking, Attitude, Motivated, Passionate. If someone has those five and can actually show me how they live them day to day, I'll take them over a person who's sold PEO for twenty years, every time. I can teach PEO. I can't teach those five. And when you're standing up a brand-new motion with no playbook yet, you want people who adapt, not people who only know one way because it worked somewhere else.</li>
          <li><b>I develop the whole team, not just my top two.</b> A team that rides on a couple of stars is fragile, and it's a miserable place to be if you're anyone else. So the bar is the same for everybody, and I hold it even when holding it is a pain. Culture isn't the speech you give at kickoff, it's what you're willing to put up with day to day. That's how my team ended up the only one in the country to put 100 percent of its consultants in President's Club, not two heroes and a long tail.</li>
          <li><b>I coach off the tape before I ever reach for a PIP.</b> I had a rep who came out of Big Four consulting, sharp guy, and he spent four months proposing deals and losing them. Easy read was to question his effort, but his calls said something else: he was going way too deep, over-prepping every meeting, and losing the prospect halfway through. So I coached him to simplify and handed him the formula my team had built over years. He was stubborn as hell about it, his way was better, we went back and forth more than once, until I finally said: run it my way one time, and if we lose this deal, go back to doing it however you want. He took the deal my way. It closed. He's now one of the top reps in the country. That taught me two things I don't forget: teach the right way from day one so nobody spends four months learning it the hard way, and your reps win or lose on the tools you put in their hands. That second one is a big reason building enablement here excites me.</li>
          <li><b>Direct and kind are the same thing, done right.</b> The kindest thing I can do for a rep who's slipping is tell them straight, early, and then build the plan with them instead of dropping it on them. Sugarcoating it isn't being nice, it's just ducking the conversation, and it makes the real one worse when it finally comes.</li>
          <li><b>I'd build comp with HR and finance, not on my own.</b> I've watched comp plans pay for logos and hand us logos that churned, and I've watched the right plan flip a team's behavior almost overnight. I'd build the PEO plan with you and finance once I actually understand the margin and the CAC, and I'd build it to reward profitable, retainable WSEs, not just signatures on paper.</li>
          <li><b>I want an HRBP who's a partner, not a signature.</b> The best HR partner I ever had told me a hire was a mistake before I could see it myself, and she was right. That's what I'm after, someone who catches the patterns I can't see from inside the team. First 90 days, that's where I'd lean on you hardest: the hiring profile, how we handle performance, and getting comp right the first time instead of fixing it later.</li>
        </ul>
      </div>
    </div>
  );
